...
if (isset($_REQUEST)){
    ...
    if ( $command == "menu" ) {
        ...
        try {
            ...
            $client->ticket($ticket_id)->update(array(
                'comment' => array(
                        'public' => true,
                        'body' => $remainder
                        )
            ...
            $response->message("Your ticket has been updated.  We'll get on it as soon as we can.");

        } catch (Exception $e) {
            $response->message("I could not find a ticket with the ID '" . $command . "'.  Are you sure thats the right ID?");
        }       
    } else {
        $response->message("Hello and thanks for the message.  Unfortunately I did not quite understand what you needed.  Try sending the word 'menu' for the list of commands.");
    }
}
